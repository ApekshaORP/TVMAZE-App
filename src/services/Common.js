...
export const getGener = async () => {
    ...
    showList&& showList.data.map(gener => { gener.genres.map((gener) => arr.push(gener));
        return arr;
    });
...
export const topShows = async () => {
    var showList = await getShows();
    var list = showList&&showList.data&&showList.data.sort((a, b) => b.rating.average - a.rating.average);
    return list;
};
//get Search shows
